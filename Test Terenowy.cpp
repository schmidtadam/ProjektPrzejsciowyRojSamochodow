...
//Platformy i belki
dReal sides[3] = {Ilosc_Aut*2.2,0.4,0.05};
...
static void simLoop (int pause)
{
	...
	dBodySetAngularVel(samochod[l].Opona[0].body, -3, 0, 0);
	dBodySetAngularVel(samochod[l].Opona[1].body, -3, 0, 0); 
	dBodySetAngularVel(samochod[l].Opona[2].body, -3, 0, 0);
	dBodySetAngularVel(samochod[l].Opona[3].body, -3, 0, 0);
}
...
void WartosciPoczatkowe() // Wylosowanie katow, ustawienie belek i platform
{
	...
	for (int l=1;l<(Ilosc_Aut-1);l++) //Losowanie kolorow - NIE DZIALA
	{
		samochod[l].czerwony=(rand()%100)/100;
		samochod[l].zielony=(rand()%100)/100;
		samochod[l].niebieski=(rand()%100)/100;
	}
...

{
	...
	samochod[licznik].radius=((rand()%20)+20)*0.01;
...
{			
	...
	dMass z6; 
		...
		dBodySetPosition (Rama[1].body,q,x+WymiaryRama[1]*sin(90-kat)-WymiaryRama[2]*sin(kat),y+0.5*wysokosc);
	...
	dMass z8;
		...
		dBodySetPosition (Rama[3].body,q,x+WymiaryRama[1]*sin(90-kat)-WymiaryRama[2]*sin(kat),y2+radius+0.5*wysokosc+WymiaryPoprz[2]);
